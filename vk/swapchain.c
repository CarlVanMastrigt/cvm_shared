...
static inline int cvm_vk_swapchain_create(cvm_vk_surface_swapchain * swapchain)
{
    ...
    swapchain->fallback_present_queue_family=swapchain->device->fallback_present_queue_family_index;
    ...
    CVM_VK_CHECK(vkCreateSwapchainKHR(swapchain->device->device,&swapchain_create_info,NULL,&swapchain->swapchain));
    ...
    CVM_VK_CHECK(vkGetSwapchainImagesKHR(swapchain->device->device,swapchain->swapchain,&swapchain->image_count,NULL));
    VkImage * swapchain_images=malloc(sizeof(VkImage)*swapchain->image_count);
    CVM_VK_CHECK(vkGetSwapchainImagesKHR(swapchain->device->device,swapchain->swapchain,&swapchain->image_count,swapchain_images));
    ...
    if(swapchain->max_image_count < swapchain->image_count)
    {
        ...
        {
            swapchain->image_acquisition_semaphores[i+1]=cvm_vk_create_binary_semaphore(swapchain->device);

            presentable_image=swapchain->presenting_images+i;

            presentable_image->present_semaphore=cvm_vk_create_binary_semaphore(swapchain->device);

            presentable_image->present_acquire_command_buffers=malloc(sizeof(VkCommandBuffer)*swapchain->device->queue_family_count);
            for(j=0;j<swapchain->device->queue_family_count;j++)
            {
    ...
    for(i=0;i<swapchain->image_count;i++)
    {
        ...
        CVM_VK_CHECK(vkCreateImageView(swapchain->device->device,&image_view_create_info,NULL,&presentable_image->image_view));
    }
...
static inline void cvm_vk_swapchain_destroy(cvm_vk_surface_swapchain * swapchain)
{
    uint32_t i,j;
    cvm_vk_swapchain_presentable_image * presentable_image;
    ...
    for(i=0;i<swapchain->image_count;i++)
    {
        ...
        /// swapchain frames
        vkDestroyImageView(swapchain->device->device,presentable_image->image_view,NULL);
...
int cvm_vk_swapchain_initialse(cvm_vk_surface_swapchain * swapchain, const cvm_vk_swapchain_setup * setup)
{
    ...
    swapchain->image_acquisition_semaphores[0]=cvm_vk_create_binary_semaphore(swapchain->device);
    ...
    CVM_VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(swapchain->device->physical_device,swapchain->surface,&format_count,NULL));
    formats=malloc(sizeof(VkSurfaceFormatKHR)*format_count);
    CVM_VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(swapchain->device->physical_device,swapchain->surface,&format_count,formats));
    ...
    CVM_VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(swapchain->device->physical_device,swapchain->surface,&present_mode_count,NULL));
    present_modes=malloc(sizeof(VkPresentModeKHR)*present_mode_count);
    CVM_VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(swapchain->device->physical_device,swapchain->surface,&present_mode_count,present_modes));
    ...
    return cvm_vk_swapchain_create(swapchain);
}
...
void cvm_vk_swapchain_terminate(cvm_vk_surface_swapchain * swapchain)
{
    ...
    cvm_vk_destroy_fence(swapchain->device,swapchain->metering_fence);

    for(i=0;i<swapchain->max_image_count+1;i++)
    {
        cvm_vk_destroy_binary_semaphore(swapchain->device,swapchain->image_acquisition_semaphores[i]);
    }
    ...
    for(i=0;i<swapchain->max_image_count;i++)
    {
        ...
        cvm_vk_destroy_binary_semaphore(swapchain->device,presentable_image->present_semaphore);
...
const cvm_vk_swapchain_presentable_image * cvm_vk_swapchain_acquire_presentable_image(cvm_vk_surface_swapchain * swapchain, bool rendering_resources_invalid, uint32_t * cleanup_index)
{
    ...
    *cleanup_index=CVM_INVALID_U32_INDEX;
    ...
    if(swapchain->rendering_resources_valid)
    {
        ...
        VkResult acquire_result=vkAcquireNextImageKHR(swapchain->device->device,swapchain->swapchain,CVM_VK_DEFAULT_TIMEOUT,acquire_semaphore,swapchain->metering_fence,&swapchain->acquired_image_index);
