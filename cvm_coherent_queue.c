...
void cvm_coherent_queue_initiasise(cvm_coherent_queue_pool * pool,cvm_coherent_queue * queue)
{
    ...
    queue->entry_mask=(((uint_fast32_t)1) << pool->capacity_exponent)-1;
...
{
    uint_fast32_t queue_index,expected_queue_index,entry_index;
    size_t entry_offset;
...
{
    uint_fast32_t queue_index,entry_index;

    queue_index = atomic_load_explicit(&queue->get_index, memory_order_relaxed);

    do if(queue_index==atomic_load_explicit(&queue->add_fence,memory_order_acquire)) return NULL;
...
//    memset(pool->entries, 7, entry_size << entry_count_exponent);
...
{
    uint_fast64_t entry_index,current_header_value,replacement_header_value,next;
    ...
    assert(entry_index < (1 << pool->capacity_exponent));
...
#warning attempt setting data in set/get directly to avoid secondary memory lookup
