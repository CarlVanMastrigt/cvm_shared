...
typedef struct cvm_transformation_frame
{
    rotor3f r;
    vec3f p;
}
cvm_transformation_frame;

#define CVM_TRANSFORM_STACK_SIZE 256

typedef struct cvm_transformation_stack
{
    cvm_transformation_frame stack[VIEW_STACK_SIZE];
    uint32_t i;
}
cvm_transformation_stack;


static inline void cvm_transform_stack_reset(cvm_transformation_stack * ts)
{
    ts->i=0;
...
static inline void cvm_transform_stack_push(cvm_transformation_stack * ts)
{
    ts->stack[ts->i+1]=ts->stack[ts->i];
    ts->i++;
}

static inline void cvm_transform_stack_pop(cvm_transformation_stack * ts)
{
    ts->i--;
}

static inline void cvm_transform_stack_get(cvm_transformation_stack * ts,float * transform_data)
{
...
static inline void cvm_transform_stack_get_scaled(cvm_transformation_stack * ts,float * transform_data,float scale)
{
    rotor3f r=ts->stack[ts->i].r;
    vec3f p=ts->stack[ts->i].p;
...
static inline vec3f cvm_transform_stack_get_transformed_position(cvm_transformation_stack * ts,vec3f p)
{
    return v3f_add(ts->stack[ts->i].p,r3f_rotate_v3f(ts->stack[ts->i].r,p));
}

static inline vec3f cvm_transform_stack_get_position(cvm_transformation_stack * ts)
{
    return ts->stack[ts->i].p;
}

static inline vec3f cvm_transform_stack_get_x_axis(cvm_transformation_stack * ts)
{
    return r3f_get_x_axis(ts->stack[ts->i].r);
}

static inline vec3f cvm_transform_stack_get_y_axis(cvm_transformation_stack * ts)
{
    return r3f_get_y_axis(ts->stack[ts->i].r);;
}

static inline vec3f cvm_transform_stack_get_z_axis(cvm_transformation_stack * ts)
{
    return r3f_get_z_axis(ts->stack[ts->i].r);
}

static inline void cvm_transform_stack_offset_position(cvm_transformation_stack * ts,vec3f offset)
{
    ts->stack[ts->i].p=v3f_add(ts->stack[ts->i].p,r3f_rotate_v3f(ts->stack[ts->i].r,offset));
}

static inline void cvm_transform_stack_rotate_around_x_axis(cvm_transformation_stack * ts,float a)
{
    ts->stack[ts->i].r=r3f_rotate_around_x_axis(ts->stack[ts->i].r,a);
}

static inline void cvm_transform_stack_rotate_around_y_axis(cvm_transformation_stack * ts,float a)
{
    ts->stack[ts->i].r=r3f_rotate_around_y_axis(ts->stack[ts->i].r,a);
}

static inline void cvm_transform_stack_rotate_around_z_axis(cvm_transformation_stack * ts,float a)
{
    ts->stack[ts->i].r=r3f_rotate_around_z_axis(ts->stack[ts->i].r,a);
}

static inline void cvm_transform_stack_rotate_around_vector(cvm_transformation_stack * ts,vec3f v,float a)
{
    ts->stack[ts->i].r=r3f_multiply(ts->stack[ts->i].r,r3f_from_v3f_and_angle(v,a));
}

static inline void cvm_transform_stack_rotate(cvm_transformation_stack * ts,rotor3f r)
{
